...
if not os.path.exists(file_path):
    raise FileNotFoundError(f'Data file not found, please check path: {file_path}')
...
def preprocess_data(data):
    # Extract molecule ID and target variable (Q_band)
    X = data.drop(['Mol_ID', 'Q_band'], axis=1)  # MODIFIED: 'Q_hand' changed to 'Q_band'
    y = data['Q_band']  # MODIFIED: 'Q_hand' changed to 'Q_band'
    mol_ids = data['Mol_ID']

    # Check and remove columns with zero variance (all-zero features)
    ...
    # Remove columns with missing values (NaNs)
...
# Split into training and testing sets (maintain 7:3 split)
...
def optimize_adaboost(X_train, y_train, X_test, y_test, n_combinations=200):
    print("\n=== AdaBoost Model Hyperparameter Optimization ===")
    ...
    base_combinations = [
        # Base combo 1: Low learning rate, fewer estimators
        {
            'n_estimators': 100,
            'learning_rate': 0.01,
            'loss': 'linear'
        },
        # Base combo 2: Medium learning rate, medium estimators
        {
            'n_estimators': 200,
            'learning_rate': 0.05,
            'loss': 'square'
        },
        # Base combo 3: High learning rate, more estimators
        {
            'n_estimators': 300,
            'learning_rate': 0.1,
            'loss': 'exponential'
    ...
    # Create a timestamp for saving intermediate results (if needed)
    ...
    for i, params in enumerate(param_combinations):
        ...
        # Update the best model based on test R²
...
# Train and optimize the AdaBoost model
best_adaboost_model, adaboost_pred, adaboost_mse, adaboost_r2, best_params = optimize_adaboost(X_train, y_train, X_test,
                                                                                               y_test)

print(f"\nFinal Testing Set R² for the Best Model: {adaboost_r2:.4f}")
