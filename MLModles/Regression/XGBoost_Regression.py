...
def optimize_xgboost(X_train, y_train, X_test, y_test, n_combinations=500):
    ...
    base_combinations = [
        ...
        # Base combo 4: High learning rate, medium depth, higher regularization
        {
            'n_estimators': 100,
            'learning_rate': 0.1,
            'max_depth': 5,
...
best_xgb_model, xgb_pred, xgb_mse, xgb_r2, best_params = optimize_xgboost(
    X_train, y_train, X_test, y_test, n_combinations=200
)

print(f"\nFinal Testing Set R² for the Best Model: {xgb_r2:.4f}")
