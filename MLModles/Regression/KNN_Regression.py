...
def optimize_knn(X_train, y_train, X_test, y_test, n_combinations=500):
    ...
    base_combinations = [
        # Base combo 1: Fewer neighbors, uniform weight, Manhattan distance
        {
            'n_neighbors': 5,
            'weights': 'uniform',
            'p': 1
        },
        # Base combo 2: More neighbors, uniform weight, Euclidean distance
        {
            'n_neighbors': 11,
            'weights': 'uniform',
            'p': 2
        },
        # Base combo 3: Fewer neighbors, distance weight, Manhattan distance
        {
            'n_neighbors': 5,
            'weights': 'distance',
            'p': 1
        },
        # Base combo 4: More neighbors, distance weight, Euclidean distance
        {
            'n_neighbors': 11,
            'weights': 'distance',
            'p': 2
...
# Train and optimize the KNN model
best_knn_model, knn_pred, knn_mse, knn_r2, best_params = optimize_knn(X_train, y_train, X_test, y_test,
                                                                      n_combinations=200)

print(f"\nFinal Testing Set R² for the Best Model: {knn_r2:.4f}")
